...
# %%
# Change setup file if running interactive environment 
def is_interactive():
...
if not is_interactive():
    ...
    parser = argparse.ArgumentParser(
        description="Run ISC analysis with different setups"
    )
    parser.add_argument(
        "-s",
        "--setup",
        type=str,
        required=True,
        help="Name of the setup module (e.g., atlas_setup)",
    )
    ...
    setup_name = "setups.debug_setup"  # Default setup when running cells
...
# %%
# Initialize parameters from setup file (project_dir/setups/*)
...
if "single-trial" in setup.model_id:
    ...
    isc_data_df = isc_utils.load_isc_data(
        base_path, folder_is="subject", model=model_is
    )  # !! change params if change base_dir
isc_data_df = isc_data_df.sort_values(by="subject")

# exclude subjects if specified
isc_data_df = isc_data_df[~isc_data_df["subject"].isin(exclude_sub)].sort_values(
    by="subject"
)
# isc_data_df = isc_data_df.sort_values(by="subject")
...
confounds_ls = isc_utils.filter_and_rename_confounds(
    dct_conf_unsorted, subjects, model_is
)
...
if keep_n_sub is not None:
    isc_data_df = isc_data_df[
        isc_data_df["subject"].isin(isc_data_df["subject"].unique()[:keep_n_sub])
    ]
    subjects = isc_data_df["subject"].unique()
    n_sub = len(subjects)
...
result_paths = {  # for all analyses results
    "isc_results": {},
...
for i, cond in enumerate(conditions):
    subject_file_dict[cond] = isc_utils.get_files_for_condition_combination(
        subjects, task_to_test[i], isc_data_df
    )
...
for cond in conditions:
    ...
    save_csv = pd.DataFrame.from_dict(
        subject_file_dict[cond], orient="index", columns=["func_path"]
    )
    save_csv.to_csv(os.path.join(cond_folder, f"setup_func_path_{cond}.csv"))
    isc_utils.save_data(
        os.path.join(cond_folder, f"setup_func_path_{cond}.pkl"),
        subject_file_dict[cond],
    )
...
if apply_mask == "lanA800":
    ...
    full_mask = nib.load(
        os.path.join(project_dir, "masks/lipkin2022_lanA800", "LanA_n806.nii")
    )
    ...
    print(
        f"Creating inverse mask: regions outside LanA800 with prob threshold = {prob_threshold}"
    )
    full_mask = nib.load(
        os.path.join(project_dir, "masks/lipkin2022_lanA800", "LanA_n806.nii")
    )
    ...
    mask_path = os.path.join(
        results_dir, f"outside_lanA800_{prob_threshold}thresh.nii.gz"
    )
...
plot_roi(
    ...
    display_mode="ortho",
    draw_cross=False,
    output_file=os.path.join(results_dir, "resamp_mask_used_isc.png"),
)
...
if "voxelWise" in atlas_name or atlas_name == "Difumo256":
    ...
    tian_sub_cortical = nib.load(
        os.path.join(
            project_dir,
            "masks/Tian2020_schaeffer200_subcortical16/Schaefer2018_200Parcels_17Networks_order_Tian_Subcortex_S1.dlabel.nii.gz",
        )
    )
    tian_data = tian_sub_cortical.get_fdata()

    # Load the labels from a text file
    labels_tian16 = os.path.join(
        project_dir,
        "masks/Tian2020_schaeffer200_subcortical16/Schaefer2018_200Parcels_17Networks_order_Tian_Subcortex_S1_label.txt",
    )
    with open(labels_tian16, "r") as f:
        tian16_labels = [line.strip() for line in f][::2][
            0:16
        ]  #!! getting only labels, only 16 !!
    ...
    combined_data[tian_data > 0] = (
        tian_data[tian_data > 0] + n_rois
    )  # Avoid index collision
    combined_img = nib.Nifti1Image(
        combined_data, affine=atlas.affine, header=atlas.header
    )
    nib.save(
        combined_img,
        os.path.join(
    ...
    isc_utils.save_json(
        os.path.join(results_dir, "roi_labels_dict.json"), roi_labels_dict
    )
    ...
    plotting.plot_roi(
        combined_img,
        colorbar=True,
        output_file=os.path.join(results_dir, "schaefer200_tian16_combined.png"),
        title="Schaefer200 + Tian16 combined",
    )

elif "SENSAAS" in atlas_name:
    atlas_native = nib.load(
        "/data/rainville/dSutterlin/projects/ISC_hypnotic_suggestions/masks/sensaas/SENSAAS_MNI_ICBM_152_2mm.nii"
    )
    atlas_data = pd.read_csv(
        "/data/rainville/dSutterlin/projects/ISC_hypnotic_suggestions/masks/sensaas/SENSAAS_description.csv"
    )
    atlas = qc_utils.resamp_to_img_mask(atlas_native, ref_img)
    ...
    atlas_data["annot_abbreviation"] = (
        atlas_data["Abbreviation"] + "_" + atlas_data["Hemisphere"]
    )
...
# reload(utils)
APM_subjects = [
    "APM" + sub[4:] for sub in subjects
]  # make APMXX format instead of subXX
print(APM_subjects)
...
X_pheno, group_labels_df, sub_check = isc_utils.load_process_behav(
    phenotype, y_interest, setup, sub_check
)
...
if transform_imgs == True:
    ...
    elif atlas_name.split("-")[0] in [
        "schafer",
        "schafer_tian",
        "SENSAAS",
    ]:  # == f'schafer{n_rois}_2mm':
    ...
    for cond in conditions:
        ...
        concatenated_subjects = {
            sub: concat_imgs(sub_files) for sub, sub_files in condition_files.items()
        }
        ...
        if atlas_name.split("-")[0] in [
            "voxelWise",
            "schafer",
            "schafer_tian",
            "SENSAAS",
        ]:  # in atlas_name: # == f'schafer{n_rois}_2mm':
            ...
            for i, sub in enumerate(subjects):
                ...
                transformed_data = masker.fit_transform(
                    concatenated_subjects[sub], confounds=conf
                )
                ls_voxel_wise.append(transformed_data)
                fitted_maskers[cond].append(
                    masker
                )  # Store the fitted masker for this subject
        ...
        if atlas_name == "Difumo256":
            transformed_data_per_cond[cond] = masker.fit_transform(
                concatenated_subjects.values()
            )
        ...
        print(
            f"Condition {cond} completed in {end_cond_time - start_cond_time:.2f} seconds"
        )

    end_total_time = time.time()
    print(
        f"Total time for data extraction: {end_total_time - start_total_time:.2f} seconds"
    )
    ...
    for cond in conditions:
        ...
        save_path = os.path.join(
            cond_folder, f"transformed_data_{atlas_name}_{cond}_{n_sub}sub.npz"
        )
        ...
        masker_path = os.path.join(
            cond_folder, f"maskers_{atlas_name}_{cond}_{n_sub}sub.pkl"
        )
    ...
    if setup.pre_computed != False:
        pre_computed_ts_dir = os.path.join(
            setup.project_dir, setup.results_branch, setup.pre_computed
        )

        if not os.path.isdir(pre_computed_ts_dir):
            raise OSError(
                f"Pre-computed directory {pre_computed_ts_dir} does not exist."
            )
        print(
            f"!! Loading existing data and fitted maskers from : {pre_computed_ts_dir}"
        )
    ...
    for cond in setup.conditions:
        ...
        transformed_path = os.path.join(
            load_path, f"transformed_data_{atlas_name}_{cond}_{n_sub}sub.npz"
        )
        transformed_data_per_cond[cond] = np.load(transformed_path)[
            "arr_0"
        ]  # utils.load_pickle(os.path.join(load_path, transformed_path))
        fitted_mask_file = f"maskers_{atlas_name}_{cond}_{n_sub}sub.pkl"
        fitted_maskers[cond] = isc_utils.load_pickle(
            os.path.join(load_path, fitted_mask_file)
        )
...
sub_to_remove = isc_utils.identify_zero_variance_subjects(
    transformed_data_per_cond, subjects
)
...
# %%
# save transformed data and maskers
save_timeseries = os.path.join(setup.results_dir, "QC_flat_rois")


# %%
# ==Format timeseries data for ISC==
...
for cond, data in transformed_data_per_cond.items():
    ...
    if isinstance(data, list):
        ...
        data_3d = data
    assert (
        data_3d.shape[-1] == n_sub
    ), f"Data shape {data_3d.shape} does not match number of subjects {n_sub}"
    transformed_formated[cond] = data_3d

transformed_data_per_cond = transformed_formated  # replace with 3d arrays

# %%
# Plot timeseries per ROI
# %%
reload(visu_utils)

roi_ids = [16, 76, 77, 187, 189]
roi_names = ["L_A1", "L_aMTG", "L_pMTG", "R_aMTG", "R_pMTG"]

for cond in conditions:
    save_plot = os.path.join(setup.results_dir, "QC_flat_rois", cond)
    os.makedirs(save_plot, exist_ok=True)

    for roi_id, roi_name in zip(roi_ids, roi_names):

        visu_utils.plot_roi_timeseries(
            transformed_data_per_cond[cond][:, roi_id, :],
            region_label=roi_name + f" (roi_id)",
            save_to=os.path.join(save_plot, f"{roi_name}_{cond}_timeseries.png"),
        )
        visu_utils.plot_roi_timeseries(
            transformed_data_per_cond[cond][:, roi_id, :],
            region_label=roi_name,
            subject_scores=X_pheno["SHSS_score"],
            save_to=os.path.join(save_plot, f"{roi_name}_{cond}_timeseries_SHSS.png"),
        )
...
if do_isc_analyses:
    ...
    for cond, data in transformed_data_per_cond.items():
        ...
        isc_results[cond] = isc_utils.isc_1sample(
            data,
        ...
        save_path = os.path.join(
            results_dir,
            cond,
            f"isc_results_{cond}_{n_boot}boot_pairWise{do_pairwise}.pkl",
        )
    ...
    for i, comb_cond in enumerate(combined_conditions):
        ...
        combined_data = np.concatenate(
            [transformed_data_per_cond[task] for task in task_to_test[i]], axis=0
        )
        ...
        isc_results[comb_cond] = isc_utils.isc_1sample(
            combined_data,
            pairwise=do_pairwise,
            n_boot=n_boot,
            side="right",
        ...
        save_path = os.path.join(
            concat_cond_save,
            f"isc_results_{comb_cond}_{n_boot}boot_pairWise{do_pairwise}.pkl",
        )
...
if do_isfc:
    ...
    for cond in combined_conditions:  # vs block wise conditions!
        ...
        isfc_results_per_cond[cond] = isc_utils.isfc_1sample(
            data, pairwise=do_pairwise, n_boot=n_boot, summary_statistic=None
        )

        save_path = os.path.join(
            isfc_save_folder,
            f"isfc_results_{cond}_{n_boot}boot_pairWise{do_pairwise}.pkl",
        )
    ...
    isfc_cols = [
        "Chge_hypnotic_depth_median_grp",
    ...
    for (
        cond,
        isfc_dict,
    ) in isfc_results_per_cond.items():  # isfc_results[cond] = subjects x roi_pairs
        ...
        for var in isfc_cols:
            ...
            group_assignment = group_labels_df[
                var
            ].values  # binary labels (0 or 1), length = n_subjects
        ...
        save_path = os.path.join(
            results_save_dir, f"{cond}_isfc_group_permutation_{n_perm}perm.pkl"
        )
...
if do_contrast_permutation:
    ...
    for i, contrast in enumerate(contrast_conditions):
        ...
        if "instrbk" in contrast:  # single trial model
            ...
            assert (
                task1.shape == task2.shape
            ), f"Data shape {task1.shape} does not match {task2.shape}"
            ...
            if contrast == "Ana-N_Ana":  # cont idx 1
                ...
                for i, trial in enumerate(Ana_trials):  # modulation trials
                    trial_data = transformed_data_per_cond[
                        trial
                    ]  # shape: TRs x ROI x subjects
                    all_trials_trimed1.append(trial_data[: tr_trim1[i], :, :])
                task1_data = np.concatenate(
                    all_trials_trimed1, axis=0
                )  # shape: (total_TRs x ROI x subjects)
        ...
        # elif contrast == "ana_run-hyper_run":
            ...
            combined_data_ls = [
                transformed_data_per_cond[task] for task in contrast_to_test[i]
            ]
        ...
        save_path = os.path.join(
            contrast_perm_save,
            f"isc_results_{contrast}_{n_perm}perm_pairWise{do_pairwise}.pkl",
        )
...
if "single-trial" in setup.model_id:
    contrast_conditions = [
        "N_ANA1_instrbk_1-N_HYPER1_instrbk_1",
        "Ana-N_Ana",
        "Hyper-N_Hyper",
    ]  # , 'Ana-N_Ana', 'Hyper-N_HYPER']
    contrast_to_test = [
        ["N_ANA1_instrbk_1", "N_HYPER1_instrbk_1"],
        ["Ana_trimmed100", "N_Ana_trimmed100"],
        ["Hyper_trimmed100", "N_Hyper_trimmed100"],
    ]
...
if do_low_high_shss_analyses:
    ...
    for g, shss_grp in enumerate(shss_grps):
        ...
        for i, contrast in enumerate(contrast_conditions):
            ...
            print(
                f"{contrast} : Repeated mesaure isc having shape : ",
                combined_data_selected.shape,
            )
            # print('group id unique : ', np.unique(group_ids_selected, return_counts=True))

            isc_grouped_selected = isc(
                combined_data_selected, pairwise=do_pairwise, summary_statistic=None
            )
            # group_ids = np.array([0] * n_sub + [1] * n_sub)
            isc_permutation_cond_contrast[contrast] = isc_utils.group_permutation(
            ...
            save_path = os.path.join(
                contrast_perm_shss,
                f"isc_results_{keep_n}sub_{contrast}_{n_perm}perm_pairWise{do_pairwise}.pkl",
            )
            isc_utils.save_data(save_path, isc_permutation_cond_contrast[contrast])
            result_paths["condition_contrast_results"][contrast] = save_path
        print(
            f"Contrasts for {shss_grp} done in {time.time() - start_grp_time:.2f} sec"
        )
...
if do_group_permutation_from_median:
    ...
    for (
        cond,
        isc_dict,
    ) in isc_results.items():  # `isc_results` should already contain ISC values
        ...
        for var in group_labels_df.columns:
            ...
            group_assignment = group_labels_df[
                var
            ].values  # Get group labels for this variable
            isc_permutation_group_results[cond][var] = isc_utils.group_permutation(
                isc_values,
                group_assignment,
                n_perm,
                do_pairwise,
    ...
    for cond, cond_results in isc_permutation_group_results.items():
        save_path = os.path.join(
            results_save_dir, f"{cond}_group_permutation_results_{n_perm}perm.pkl"
        )
...
if do_rsa:
    ...
    combined_conditions = (
        setup.combined_conditions
    )  # ['all_sugg', 'modulation', 'neutral']
    ...
    for sim_model in ["euclidean", "annak"]:
        ...
        for cond in all_conditions:
            ...
            # if cond == 'all_sugg':
            #     breakpoint()
            save_cond_rsa = os.path.join(
                rsa_save_dir, f"rsa-isc_{cond}"
            )  # make condition folder
            ...
            for behav_y in y_interest:  # repeated for each Yi
                ...
                df_subjectwise_rsa = pd.DataFrame(
                    index=range(isc_pairwise.shape[0]), columns=atlas_labels
                )
                ...
                for col_j in range(isc_pairwise.shape[1]):  # j ROIs
                    ...
                    rsa_results = isc_utils.matrix_permutation(
                        sim_behav,
                        isc_roi_vec,
                    ...
                    values_rsa_perm[roi_name] = {
                        "correlation": rsa_results["correlation"],
                        "p_value": rsa_results["p"],
                    }
                ...
                csv_path = os.path.join(
                    save_cond_rsa,
                    f"{behav_y}_rsa_isc_{sim_model}simil_{n_perm_rsa}perm_pvalues.csv",
                )
                df_rsa.to_csv(csv_path)
                dist_path = os.path.join(
                    save_cond_rsa,
                    f"{behav_y}_rsa_isc_{n_perm_rsa}perm_distribution.pkl",
                )
