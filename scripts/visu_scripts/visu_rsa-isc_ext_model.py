...
save_path = "/data/rainville/dSutterlin/projects/ISC_hypnotic_suggestions/results/imaging/RSA/isc-RSA_ext_conds_sugg-pain_tian216_2tails"
save_figures = os.path.join(
    project_dir,
    "results/imaging/RSA/isc-RSA_ext_conds_sugg-pain_tian216_2tails/post-hoc_figures",
)
...
def plot_behavioral_values(
    Y,
    cols_names,
    index_names,
    cbar_label="scores",
    title="Behavioral Features Heatmap",
    cmap="viridis",
):
    plt.figure()
    plt.imshow(Y)
...
plot_behavioral_values(
    Y_pain, pain_cols, Y.index, title="Pain modulation response patterns"
)

# %%
# RSA SHSS ~ other hypnotic features : Similar SHSS --> similar cognitive states?
scaler = StandardScaler()
hypnotic_cols = [
    "Chge_hypnotic_depth",
    "Mental_relax_absChange",
...
y_shss = scaler.fit_transform(
    np.array(Y["SHSS_score"].values, dtype=float).reshape(-1, 1)
)
Y_hypno = scaler.fit_transform(np.array(Y[hypnotic_cols].values, dtype=float))

cosine_sim_hypno = isc_utils.compute_behav_similarity(
    Y_hypno, metric="cosine", vectorize=False
)
cosine_vec_hypno = isc_utils.compute_behav_similarity(
    Y_hypno, metric="cosine", vectorize=True
)
...
sim_shss_vec = isc_utils.compute_behav_similarity(
    y_shss, metric="euclidean", vectorize=True
)
sim_shss_vec_annak = isc_utils.compute_behav_similarity(
    y_shss, metric="annak", vectorize=True
)
sim_shss_mat = isc_utils.compute_behav_similarity(
    y_shss, metric="euclidean", vectorize=False
)
plot_simmat(
    sim_shss_mat,
    title="Euclidean",
...
r, p, dist_annak = isc_utils.matrix_permutation(
    sim_shss_vec_annak,
    cosine_vec_hypno,
    n_permute=10000,
    metric="spearman",
...
print(f"Spearman correlation between SHSS and Annak hypno: {r:.4f}, p-value: {p:.4f}")
...
atlas = nib.load(
    os.path.join(
        project_dir,
        "masks/Tian2020_schaeffer200_subcortical16/combined_schaefer200_tian16_DSG.nii.gz",
    )
)
id_labels_dct = isc_utils.load_json(
    os.path.join(
        project_dir,
        "masks/Tian2020_schaeffer200_subcortical16/roi_labels_dict_DSG.json",
    )
)
# remove background
id_labels_dct.pop("0")
...
rsa_isc = isc_utils.load_pickle(
    "/data/rainville/dSutterlin/projects/ISC_hypnotic_suggestions/results/imaging/RSA/isc-RSA_ext_conds_sugg-pain_tian216_2tails/rsa_isc-sugg_isc-pain_10000perm.pkl"
)
...
for cond in ["ana_run", "hyper_run"]:
    ...
    isc_pairwise[cond] = pd.DataFrame(
        isc_utils.load_pickle(isc_path)["isc"], columns=labels
    )
...
for cond in ["ana_run", "hyper_run"]:  # , 'HYPER', 'ANA']: #, 'all_sugg', 'neutral']:
    ...
    print("cond", cond)
    rsa_df = rsa_isc[model][cond].sort_index(
        ascending=True
...
for roi_idx, roi in zip(sig_roi["ROI"], sig_roi["Label"]):
    ...
    plot_simmat(
        ranked_isc_mat, title=f"ISC {roi}", title_id=roi, y_name="SHSS", cmap=cmap
    )
...
visu_utils.plot_scatter_legend(
    vec_rsa_nn["spearman_r"],
    vec_rsa_annak["spearman_r"],
    grp_id=yeo7_net,
    var_name=["Eucledian-based ISC ", "AnnaK-based ISC"],
    title=f"Eucledian vs Annak-based IS-RSA per region ",
    save_path=None,
)
...
print(
    f"T-test between Eucledian and AnnaK-based IS-RSA: t-statistic = {t_stat:.4f}, p-value = {p_val:.4f}"
)
...
print(
    f"Wilcoxon test between Eucledian and AnnaK-based IS-RSA: W-statistic = {w_stat:.4f}, p-value = {p_val_w:.4f}"
)
...
for roi_idx, roi in zip(sig_roi["ROI"], sig_roi["Label"]):
    ...
    visu_utils.jointplot(
        Y["SHSS_score"],
        median_per_row,
        x_label="SHSS score (ranked)",
        y_label=f"ISC {roi} (ranked)",
